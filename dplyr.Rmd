...
\begin{frame}[fragile]
  \frametitle{What about writing it like this:}
<<eval=FALSE>>=
add(2*egg, sugar) %>%
   whisk() %>%
   add(milk) %>%
...
\begin{frame}
  \frametitle{Pipe operator `\%>\%`}
    \begin{itemize}
    \item Takes output of previous function (expression)
    \item Feeds it as the first argument of the following function
      \begin{itemize}
      \item Use `.` placeholder if you want it not as first but
        another argument
      \end{itemize}
    \end{itemize}
\end{frame}
