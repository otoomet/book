...
to live with coding errors and to learn to fix them.
...
you don't know what the `if` statement or a variable
is supposed to do.

This is less of a problem when you are debugging your own code but may
be quite laborious if it has been written by someone else.  The issue
...
debugging.  You just insert printing command into your code that
...
cannot easily otherwise debug.  For instance, if your code has to run
in a docker container on a restricted-access server, then it is
extremely laborious to replicate the (restricted) working environment
