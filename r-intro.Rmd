...
for working with data. It is one of the most popular data science
tools because of its ability to efficiently perform statistical
analysis, implement machine learning algorithms, and create data
visualizations. R is the primary programming language used throughout
this book, and understanding its foundational operations is key to
being able to perform more complex tasks.

This chapter is primarily designed for readers who have little to no
experience with programming, and hence we discuss topics like
variables and data types in a rather thorough way.  If you have
programming experience, you may quickly skim through this chapter and
just learn the basic R syntax and how to use RStudio.
...
computers, such as by writing syntactically-precise instructions at
the command-line. Programming in R will work in a similar manner: you
will write instructions R language and syntax (not in shell language as in shell), which the computer will **interpret** as instructions for how to work with data.

However, as projects grow in complexity, it will become useful if you
can write down all the instructions in a single file, and then tell
the computer to _execute_ all of those instructions at once. This list
of instructions is called a **script** or **program** (or **code**).  Writing
scripts is called **programming**.  Executing or "running" a script will cause each instruction (line of code) to be run _in order, one after the other_, just as if you had typed them in one by one. Writing scripts allows you to save, share, and re-use your work. By saving instructions in a file (or set of files), you can easily check, change, and re-execute the list of instructions as you figure out how to use data to answer questions. And, because R is an _interpreted_ language rather than a _compiled_ language like Java, R programming environments will also give you the ability to execute each individual line of code in your script if you desire (though this will become cumbersome as projects become large).
...
The primary way to use R in this course is through _RStudio_ (see
below).  However, R can be used also without [RStudio](#running-r-rstudio).
...
With R installed, you can start an interactive R session on a Mac (or linux) by typing `R` into the terminal (to run the `R` program), or on Windows by running the "R" desktop app program. This will start the session and provide you with lots of information about the R language:

![An interactive R session running in the terminal.](img/r-intro/r-interactive-session.png "interactive R session")

Notice that this description also includes suggestions on what to do next_&mdash;most importantly `"Type 'q()' to quit R."`.
...
- Note that RStudio also includes an interactive console that provides the exact same functionality.
...
On Windows, you need to tell the computer where to find the `R.exe`
and `RScript.exe` programs to execute&mdash;that is, what is the
**path** to these programs.  You can do this by specifying the _absolute path_ to the R program when you execute it, for example:

![Using RScript from a Windows shell](img/r-intro/rscript-terminal-windows.png "Rscript from windows shell")

If you plan to run R from the command-line regularly (_not_ a
requirement for this course), a better solution is to add the folder
containing these programs to your computer's [**`PATH`
variable**](https://en.wikipedia.org/wiki/PATH_(variable)). This is a
_system-level_ variable that contains a list of folders that the
computer searches when looking for programs. The reason the computer knows where to find the `git.exe` program when you type `git` in the command-line is because that program is "on the `PATH`".
...
However, unless you have good reasons, we recommend you to use RStudio
...
- **Console**: The bottom-left pane is a console, the R command line
  for entering R commands. This is identical to an inetractive session
  you'd run on the command-line, in which you can type and execute one
  line of code at a time. The console will also show the printed
  results from executing the code you execute from the Script pane.
  
  Normally you use console for quick computations and short sequences
  of 1-2 lines of code.  Longer blocks of code is usually easier to
  as script.

    - _Protip:_ just like with the command-line, you can **use the up arrow** to easily access previously executed lines of code.

- **Script**: The top-left pane is a simple text editor for writing
  your R code, markdown, and other files.  While it is not as powerful
  as dedicated text editors
  like Atom, it will colorize code (this is called _syntax coloring_), "auto-complete" text, and allows you to easily execute your code. Note that this pane is hidden if there are no open scripts; select `File > New File > R Script` from the menu to create a new script file.

    In order to execute (run) the code you write, you have two options:

    1. You can execute a section of your script by selecting
       (highlighting) the desired code and pressing the "Run" button
       (keyboard shortcut: `Ctrl + Enter`). If no lines are selected,
       this will run the line currently containing the cursor.  This
       is a good way to run just a selected section of your code.

    2. You can execute an entire script by using the `Source` command
       to treat the current file as the "source" of code. Press the
       "Source" button (`Ctrl + Shift + Enter`) to do so.  This is a
       good approach if you want to run your entire script.
	   
	   If you check the "Source on save" option, your entire script
       will be executed every time you save the file (this may or may
       not be appropriate, depending on the complexity of your script
       and its output).  There are more options.
...
reorganize the panes if you wish.  The most useful options to
alternatives are _focusing_ and _zooming_.  Focusing means moving your
mouse cursor and input into a particular pane, e.g. `Ctrl + 1` makes
the script pane active and `Ctrl + 2` makes console pane active.
Using keyboard shortcuts to move your focus is much faster than
grabbing mouse.

Zooming is a bit similar to focus, just it also hides the other
panes.  `Ctrl + Shift + 1` zooms to script, `Ctrl + Shift + 2` zooms
to console, and `Ctrl + Shift + 0` restores the original 4-pane view.
See the `View > Panes` menu and options therein.  Zooming to
individual panes is very useful if you are working on a small screen.


## Basic R

Here we introduce the very basics of R through R console.  Although
the same commands can also be written as a script, 1-2 line long
commands are typically easier to just be typed on the console (the
lower-left pane in RStudio).

R Console is similar to command shell, just it understands R commands
and not shell commands.  We can start with simple arithmetic.  Write
`1 + 1` in the R command prompt and hit enter (in a similar fashion as
in shell).  R replies with `[1] 2`.  Below we write these steps as:
```{r}
1 + 1
```
The first block shows the commands you issue in R console, underneath
is `##` followed by the R's reply (the answer).  The R's reply
contains the answer, `2`, and a marker `[1]`.  The marker means that
one command may produce many answers and this is the first answer.

This is how we can use R as a powerful calculator.  The other
arithmetic operations are pretty easy and intuitive: `-` for
subtraction, `*` for multiplication, `/` for division and `^` for
exponentiation.  Only exponentiation is somewhat non-standard,
different programming languages have different habits here.  R knows
that multiplication must be done before addition, if you want the
opposite then you need parenthesis:
```{r}
1 + 2*3
(1 + 2)*3
```
Obviously, we do not want to use a complex statistical language for
such simple computations.  But let's compute length of light-year in
kilometers instead--this is much harder to do manually.
Light-year is the distance light, moving 300,000
km/s, covers in one year:
```{r}
300000*60*60*24*365
```
Here we take the speed of light, and multiply it by seconds in minute
(60), minutes in hour (60), hours in day (24) and days in year (365).
R prints the answer in _exponential form_, it must be understood as
$9.46\cdot 10^{12}$, i.e. almost 10 trillion kilometers. 


## Variables
Since computer programs involve working with lots of _information_, we
...
There six "basic types" (called _atomic data types_) for data in R:
...
- **Character**: Character data stores _strings_ of characters (things you type with a keyboard) in a variable. You specify that some information is character data by surrounding it in either single quotes (**`'`**) or double quotes (**`"`**). To comply with the [style guide](http://style.tidyverse.org/syntax.html#quotes), we'll consider it best practice to use **double quotes**.
...
- **Complex**: Complex (imaginary) numbers have their own data storage
    type in R, are are created using the `i` syntax: `complex_variable
    <- 1 + 2i`. We will not be using complex numbers in this course.
...
symbol (**`#`**). Everything from the `#` until the end of the line is
a comment.  It is common to put descriptive comments immediately above
the code it describes, and sometimes immediately aftewards.  One can also put very short notes at the end of the line of code (preferably following two spaces):
...
(You may have seen this `#` syntax and commenting behavior from the command-line and git chapters. That's because the same syntax is used in a Bash shell!)


## Producing output: `cat` and `print` {#producing-output-cat-print}

When you just compute on R console, or even when you write small
scripts, it is not necessary to dedicate any effort to _output_.  The
results are automatically printed.  This is a common behavior in R
console: the last result will be printed.  However, when writing
longer programs, one may want to see more results than the last one,
and maybe also add some explanatory notes.  This is where printing
comes in.

R has two main printing commands: `cat` and `print`.  `cat` is useful
if you want to print simple objects, but potentially more than one,
such as strings, numbers, and
explanatory text.  `print` can output complex objects but only one at
time.
...
```
This short script computes the length of light-year and prints it with
a small informative message.  Alternatively, we can just compute this
number and let R console to automatically print it:
```{r}
300000*60*60*24*365
```
Why should we use `cat` then?  The automatic printing is good enough
if you work interactively on console, or just run very short code
snippets.  But if the code is not run on R console the number may not
even be printed.  Alternatively, if the script computes and prints
many results, the user gets easily confused what do these numbers
mean.  So it is a good habit to output your results together with a
brief explanation.

The syntax of `cat` is pretty simple: it takes a list of arguments,
texts, variables and numbers you want to print.  One very useful
symbol you may want to add is _newline_ character "\n".  This forces
printing to jump to the next line:
```{r}
...
`print` is somewhat similar to `cat` but designed to output more
complex objects, such as [vectors](#vectors), [lists](#lists), and
[data frames](#data-frames).  Print may produce multi-line output but
it does not allow to add explanatory messages.  You have to _cat_ the
message and _print_ your complex object thereafter.

Obviously, output does not have to be printed on console, it may also
be sent to a file, or uploaded to internet, or played as audio
instead.  But whatever the exact format, it is important to ensure the
user has enough information to understand what the output is.
